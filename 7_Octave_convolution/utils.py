...
def plot_accuracy_and_loss(history):
    ...
    axs[0].plot(history["acc"], label="Training accuracy")
...
def plot_smoothed_acc_and_loss(history, factor=0.8):
    fig, axs = plt.subplots(1, 2, figsize=(15, 5))
    axs[0].plot(
        smooth_curve(history["acc"], factor=factor), label="Training accuracy"
    )
    axs[0].plot(
        smooth_curve(history["val_acc"], factor=factor),
    ...
    axs[1].plot(
        smooth_curve(history["loss"], factor=factor), label="Training loss"
    )
    axs[1].plot(
        smooth_curve(history["val_loss"], factor=factor),
